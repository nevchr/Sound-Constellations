# Sound Constellations — development guide
...
The command builds a guided NSIS setup executable, a portable executable, checksums, and a manifest under `release/0.1.0/`. Both editions include Electron and the local decoder. `pnpm build:dir` produces the unpacked application under `release/0.1.0/win-unpacked/`. End users do not install development dependencies. The initial executables are unsigned. See [installation instructions](INSTALL.md) and [third-party notices](../THIRD_PARTY_NOTICES.md). Packaging explicitly disables automatic publishing.
...
Run `pnpm test:installed` after building to verify the installer and portable edition. It refuses to replace an existing installation, installs into a separate temporary directory outside the checkout, tests the bundled app with a restricted PATH, and uninstalls only the installation it created. Evidence is written under `test-results/`. It does not substitute for testing on a clean Windows virtual machine.
